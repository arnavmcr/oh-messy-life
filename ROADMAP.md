...
| 2026-08-02 | Connected-node hover pulse: trigger is "hovered node has neighbors," not transition-based | Simpler to reason about than comparing previous/current hover; matches the literal ask (pulse sent from the main node to all nodes involved) |
| 2026-08-02 | Connected-node hover pulse: fixed animation duration, not fixed speed, for traveling dots | SVG's duration-based `<animateMotion>` makes every neighbor's dot arrive simultaneously regardless of edge length, per explicit requirement |
